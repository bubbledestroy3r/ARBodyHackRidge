...
class foodLogTableViewController: UITableViewController {
    var m : [Meal] = []
    var mealManager = [String]()
    
    override func viewWillAppear(_ animated: Bool) {
        getData()
        for i in m
        {
            mealManager.insert(i.meals!, at: 0)
        }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = mealManager[indexPath.row]
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        ...
        {
            context.delete(m[indexPath.row])
            (UIApplication.shared.delegate as! AppDelegate).saveContext()
            do {
                m = try context.fetch(Meal.fetchRequest())
            } catch {
